...
<section class="container-fluid p-5 container-api">
  ...
  <form action="createMovie.php" method="post" class="form-peliculas" enctype=multipart/form-data>
    ...
    <input type="text" name="nombre" value="<?php echo isset($form_data['nombre']) ? $form_data['nombre'] : ($create ? $movieEdit['nombre'] : ''); ?>" />
    <?php if (isset($errores['nombre'])) : ?>
      <p class="text-danger fs-6 mx-5"><?php echo $errores['nombre']; ?></p>
    <?php endif; ?>

    <label for="">Descripción</label>
    <textarea name="descripcion" class="descripcion"><?php echo isset($form_data['descripcion']) ? $form_data['descripcion'] : ($create ? $movieEdit['descripcion'] : ''); ?></textarea>
    <?php if (isset($errores['descripcion'])) : ?>
      <p class="text-danger fs-6 mx-5"><?php echo $errores['descripcion']; ?></p>
    <?php endif; ?>

    <label for="genero">Género</label>
    <input type="text" name="genero" placeholder="separar con coma los Género" value="<?php echo isset($form_data['genero']) ? $form_data['genero'] : ($create ? $movieEdit['genero'] : ''); ?>" />
    ...
    <select name="calificacion" id="calificacion">
      <option value="" disabled <?php echo !$create && !isset($form_data['calificacion']) ? 'selected' : ''; ?>>Selecciona una calificación</option>
      <?php
      $selectedCalificacion = isset($form_data['calificacion']) ? (int)$form_data['calificacion'] : ($create ? (int)$movieEdit['calificacion'] : null);
      for ($i = 1; $i <= 10; $i++) : ?>
        <option value="<?php echo $i; ?>" <?php echo $selectedCalificacion === $i ? 'selected' : ''; ?>><?php echo $i; ?></option>
      <?php endfor; ?>
    </select>
    ...
    <select name="seccion" id="seccion">
      <option value="" disabled <?php echo !$create && !isset($form_data['seccion']) ? 'selected' : ''; ?>>Selecciona una sección</option>
      <option value="list" <?php echo (isset($form_data['seccion']) && $form_data['seccion'] === 'list') || ($create && $movieEdit['seccion'] === 'list') ? 'selected' : ''; ?>>List</option>
      <option value="tendencias" <?php echo (isset($form_data['seccion']) && $form_data['seccion'] === 'tendencias') || ($create && $movieEdit['seccion'] === 'tendencias') ? 'selected' : ''; ?>>Tendencias</option>
      <option value="aclamadas" <?php echo (isset($form_data['seccion']) && $form_data['seccion'] === 'aclamadas') || ($create && $movieEdit['seccion'] === 'aclamadas') ? 'selected' : ''; ?>>Aclamadas</option>
    </select>
    ...
    <input type="number" name="anio" value="<?php echo isset($form_data['anio']) ? $form_data['anio'] : ($create ? $movieEdit['anio'] : ''); ?>" />
    <?php if (isset($errores['anio'])) : ?>
      <p class="text-danger fs-6 mx-5"><?php echo $errores['anio']; ?></p>
    <?php endif; ?>

    <label for="director">Director</label>
    <input type="text" name="director" value="<?php echo isset($form_data['director']) ? $form_data['director'] : ($create ? $movieEdit['director'] : ''); ?>" />
    ...
    <?php if (!empty($movieEdit['imagen'])) : ?>
      <img src="<?php echo !empty($movieEdit['imagen']) || file_exists('../uploads/image/' . $movieEdit['imagen']) ? $movieEdit['imagen'] : '../../asset/images/no-disponible.jpg' ?>" class="mx-5 my-4 img-form" alt="imagen pelicula">
    <?php endif; ?>
